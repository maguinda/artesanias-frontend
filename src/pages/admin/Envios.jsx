    ...
    <MainLayout showFilter={false}>
      <div className="admin-page">
        <div className="admin-page__header" style={{ flexWrap:'wrap', gap:12 }}>
          <h2 className="admin-page__title">Envíos</h2>
          ...
          <>
            <div className="admin-table-wrapper">
              <table className="admin-table">
                <thead>
                  <tr>
                    {['# Orden','Cliente','Dirección','Mensajería','Pago','Envío','Total','Estado','Cambiar'].map(h => (
                      <th key={h}>{h}</th>
                    ))}
                  </tr>
                </thead>
                      ...
                      <>
                        <tr key={o.id}
                          ...
                          <td>
                            ...
                            <div style={{ fontSize:11, color:'var(--gray-500)' }}>{o.customer_phone || ''}</div>
                          </td>
                          <td>
                            <div style={{ fontSize:13 }}>{o.shipping_address || '—'}</div>
                            {o.barrio && <div style={{ fontSize:11, color:'var(--gray-500)' }}>{o.barrio}</div>}
                            {o.city   && <div style={{ fontSize:11, color:'var(--gray-500)' }}>{o.city}</div>}
                          </td>
                          <td style={{ fontSize:13 }}>{o.shipping_company || '—'}</td>
                          <td style={{ fontSize:13, textTransform:'capitalize' }}>{o.payment_method || '—'}</td>
                          <td style={{ fontSize:13 }}>{fmt(o.shipping_cost || 0)}</td>
                        ...
                        {isOpen && (
                          <tr key={`d-${o.id}`}>
                            <td colSpan={9} style={{ padding:0, background:'#fdf8f2' }}>
                              <div style={{ padding:'12px 20px', borderTop:'1px solid var(--cream-dark)' }}>
                                ...
                                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px,1fr))', gap:8, fontSize:12, marginBottom:10 }}>
                                  {[
                                    ...
                                    ['Costo envío',fmt(o.shipping_cost || 0)],
                                    ['Total',      fmt(o.amount)],
                                    ['Notas',      o.notes || '—'],
              ...
              {filtered.length === 0 && (
                <div className="empty-state">
                  <h3>No hay órdenes {filter !== 'todos' ? `en estado "${filter}"` : search ? `para "${search}"` : ''}</h3>
                </div>
              )}
            </div>
            ...
            <p style={{ textAlign:'center', color:'var(--gray-500)', fontSize:12, marginTop:8 }}>
              Mostrando {Math.min(page * PAGE_SIZE, filtered.length)} de {filtered.length} órdenes
            </p>
          </>
        )}
      </div>
